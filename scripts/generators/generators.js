...
export class Generators {
  ...
  static applyGeneratorToEmitterConfig(options, config) {
    ...
    config.maxParticles = (canvas.dimensions.width / canvas.dimensions.size) * (canvas.dimensions.height / canvas.dimensions.size) * options.density
    ...
    if (speed !== undefined) {
      ...
      const sceneDiagonal = Math.sqrt(sceneRect.width * sceneRect.width + sceneRect.height * sceneRect.height)
      ...
      const tp = Generators._getTransposedPosition(sceneRect.x, sceneRect.y, sceneRect.width, sceneRect.height, sceneDiagonal / 4, (options.direction + 180) % 360)
