...
export class SceneWeatherLayer extends InteractionLayer {
   ...
   */
  get elevation() {
    return (canvas.weather?.elevation ?? 9999) + 2
  }
  set elevation(value) {
  ...
  async createWeatherNodes(
    ...
    Logger.debug('SceneWeatherLayer.createWeatherNodes(...)', {
      nodeDatas: nodeDatas,
      forceId: forceId,
      ...
      fireEvent: fireEvent,
      newWeatherNodes: newWeatherNodes
    })
    if (fireEvent) {
      Hooks.callAll(EVENTS.CREATE_WEATHER_NODES, {
        ...
        deletion: [],
        addition: [
          newWeatherNodes.map((newWeatherNode) => {
  ...
  async updateNodes(nodesToUpdate = [], { storeHistory = true, fireEvent = true } = {}) {
    ...
    Logger.debug('SceneWeatherLayer.updateNodes(...)', {
  ...
  async deleteWeatherNodes(
    ...
    Logger.debug('SceneWeatherLayer.deleteWeatherNodes(...)', {
    ...
    if (fireEvent) {
      Hooks.callAll(EVENTS.DELETE_WEATHER_NODES, {
        update: [],
  ...
  storeHistory(type, data) {
    ...
    if (this._history.length >= SWCONFIG.UNDO_STEPS) this._history.shift()
