...
Hooks.on(EVENTS.WEATHER_DISABLED, async (data) => {
  ...
  if (canvas['sceneweatherfx'] !== undefined) {
    await Promise.all([
      canvas.sceneweatherfx.drawParticleEffects({ soft: false }),
...
export class WeatherEffectsLayer extends CanvasLayer {
  ...
  static getFxFiltersForModel(modelData) {
    ...
    Logger.debug('WeatherEffectsLayer.getFxFiltersForModel()', {
      model: modelData,
   ...
   */
  get elevation() {
    return (canvas.weather?.elevation ?? 9999) + 1
  }
  set elevation(value) {
  ...
  async _tearDown() {
    ...
    filterContainer.filters =
      filterContainer.filters?.filter(function (objFromA) {
  ...
  async drawParticleEffects(options) {
    ...
    const stopPromise = Promise.all(
      this.activeEffects.map(async (effect) => {
        if (options.soft) {
        ...
        if (index > -1) {
          // only splice array when effect is found
    ...
    Logger.debug('waiting for emitters to clean up (softly)', {
