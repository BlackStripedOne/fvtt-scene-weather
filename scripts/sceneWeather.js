...
export class SceneWeather {
  ...
  _getWeatherModelForMode(scene) {
    ...
    Logger.debug('SceneWeather._getWeatherModelForMode(...)', {
      scene: scene,
  ...
  updateConfig() {
    ...
    if (scene === undefined) {
      Logger.error(
        'Unable to instantiate SceneWeather for non existing Scene with id ' + this.sceneId
      )
      throw new Error(
        'Unable to instantiate SceneWeather for non existing Scene with id ' + this.sceneId
      )
    ...
    if (Fal.getSceneFlag('weatherMode', null, this.sceneId) != this.weatherMode) {
      ...
      Logger.debug('SceneWeather.updateConfig() -> newMode', {
        sceneId: this.sceneId,
        prevMode: this.weatherMode,
        newMode: Fal.getSceneFlag('weatherMode', '-', this.sceneId)
      })
  ...
  static _weatherModelToExternal(model, rainMode, weatherMode = GENERATOR_MODES.WEATHER_TEMPLATE) {
    const [preciType] = Object.entries(PRECI_TYPE).find(
      ([, val]) => val === model.precipitation.type
    )
    const [cloudType] = Object.entries(CLOUD_TYPE).find(([, val]) => val === model.clouds.type)
    const [windType] = Object.entries(WIND_MODES).find(
      ([, val]) =>
        val === (weatherMode == GENERATOR_MODES.WEATHER_GENERATE ? model.wind.directionType : 0)
    )
      ...
      clouds: {
        ...
        thickness:
          weatherMode == GENERATOR_MODES.WEATHER_GENERATE
            ? Math.round(model.clouds.thickness)
            : Utils.clamp(Math.round(model.clouds.top) - Math.round(model.clouds.bottom), 0, 20000)
      },
      precipitation: {
  ...
  getWeatherSettings() {
    ...
    switch (settings.generator.mode) {
      case GENERATOR_MODES.WEATHER_TEMPLATE:
        const templateData =
          SceneWeatherState._weatherTemplates[Fal.getSceneFlag('weatherTemplate', '', this.sceneId)]
        if (templateData !== undefined) {
          settings.weather = SceneWeather._weatherModelToExternal(
          templateData,
          Fal.getSceneFlag('rainMode', RAIN_MODES.WINDDIR, this.sceneId),
          settings.generator.mode
        )
      ...
      case GENERATOR_MODES.WEATHER_GENERATE:
        ...
        break
      case GENERATOR_MODES.REGION_TEMPLATE:
        settings.region = Utils.deepClone(
        SceneWeatherState._regionTemplates[
          Fal.getSceneFlag('regionTemplate', '', this.sceneId)
        ] || {}
      )
      ...
      case GENERATOR_MODES.REGION_GENERATE:
        ...
        Fal.getSceneFlag(
          'regionSettings',
          Utils.deepClone(Fal.getSetting('defaultRegionSettings')),
          this.sceneId
        )
      )
        settings.weather = SceneWeather._weatherModelToExternal(
